...
/**
   * Ecommerce Cart Functionality - Sửa lỗi bởi Gemini
   * Đồng bộ giữa giao diện và Server PHP
   */
 /**
   * Ecommerce Cart Functionality - Đã tối ưu
   * Chỉ giữ lại xử lý nút Xóa, việc tăng giảm để PHP xử lý qua hàm changeQty
   */
  function ecommerceCartTools() {
    const removeButtons = document.querySelectorAll('.remove-item');

    // XỬ LÝ NÚT XÓA (Gửi lệnh về PHP thông qua form)
    removeButtons.forEach(btn => {
      btn.addEventListener('click', function (e) {
        // Chặn hành động mặc định của template
        e.preventDefault(); 
        e.stopPropagation();

        // Tìm form xóa bao quanh nút này
        const form = btn.closest('form');
        
        if (form) {
          if (confirm('Bạn có chắc chắn muốn xóa sản phẩm này khỏi giỏ hàng?')) {
            form.submit(); // Gửi form POST thực sự về cart.php để xóa trong Session
          }
        }
      });
    });

    // Lưu ý: Các nút tăng/giảm số lượng đã có thuộc tính onclick="changeQty(...)" 
    // trực tiếp trong file HTML/PHP nên không cần viết thêm logic ở đây nữa.
  }
 ...
 function initOnePageCheckout() {
    ...
    paymentOptions.forEach(option => {
      option.addEventListener('change', function () {
        ...
        const targetDetails = document.getElementById(`${paymentId}-details`);
        if (targetDetails) targetDetails.classList.remove('d-none');
      });
    });
    ...
    if (checkoutForm) {
      checkoutForm.addEventListener('submit', function (e) {
        // 1. Tạm dừng để kiểm tra dữ liệu bằng Javascript
        e.preventDefault();
        ...
        if (!isValid) {
          // Nếu có lỗi, cuộn đến ô lỗi đầu tiên
          if (firstInvalidField) {
            firstInvalidField.scrollIntoView({ behavior: 'smooth', block: 'center' });
            firstInvalidField.focus();
          }
        } else {
          // 2. QUAN TRỌNG: Nếu dữ liệu hợp lệ, thực hiện gửi Form thật về PHP
          // Xóa bỏ đoạn "Simulate redirect" cũ và thay bằng lệnh submit thực
          
          // Thay đổi nội dung nút bấm để người dùng biết đang xử lý
          ...
          // Gửi form về server (checkout.php sẽ nhận được $_POST)
          checkoutForm.submit(); 
        }
      });
...
document.addEventListener("DOMContentLoaded", function () {
 ...
 // === SỰ KIỆN ===
// Chỉ chạy các lệnh này NẾU các phần tử tồn tại trên trang hiện tại
...
// SỬA ĐOẠN NÀY: Chỉ khởi tạo nếu có phần tử trên trang
